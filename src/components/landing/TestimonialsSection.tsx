...
const StarRating = ({
  ...
  return <div className="flex items-center">
      {[1, 2, 3, 4, 5].map(star => <Star key={star} className={`h-4 w-4 ${star <= rating ? 'text-gold fill-gold' : 'text-gray-300'}`} />)}
    </div>;
};
      ...
      <div className="max-w-7xl mx-auto px-4 mb-12">
        <div className="flex items-center justify-center mb-8">
          <div className="flex -space-x-3 mr-4">
            {testimonials.slice(0, 5).map((testimonial, index) => <Avatar key={index} className="h-10 w-10 border-2 border-navy">
                <AvatarImage src={testimonial.avatarUrl} alt={testimonial.name} />
                <AvatarFallback className="bg-navy-light text-gold">
                  {testimonial.name.substring(0, 2)}
                </AvatarFallback>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {testimonials.slice(0, 8).map((testimonial, index) => <div key={`testimonial-${index}`}>

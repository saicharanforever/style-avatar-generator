          ...
          {testimonials.map((testimonial) => (
            ...
            >
              <div className="flex items-center mb-4">
                ...
                />
                <div className="ml-3">
                  <h3 className={`font-bold ${theme === 'dark' ? 'text-gold' : 'text-white'}`}>{testimonial.name}</h3>
                  <p className={`text-sm ${theme === 'dark' ? 'text-white/70' : 'text-white/70'}`}>{testimonial.business}</p>
                </div>
              </div>
              
              <div className="flex items-center mb-3">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star key={i} className={`h-4 w-4 ${theme === 'dark' ? 'text-yellow-300' : 'text-yellow-300'} fill-current`} />
                ))}
              </div>
              
              <p className={theme === 'dark' ? 'text-white/90' : 'text-white'}>"{testimonial.text}"</p>
            </div>
          ))}
        </div>
      </div>
